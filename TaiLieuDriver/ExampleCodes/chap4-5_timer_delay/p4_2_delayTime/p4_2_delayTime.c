/*
 * char_driver_4.c - character driver hoan chinh. Tich hop Delay time
 */
...
#define DRIVER_DESC   "Tich hop delay time vao character driver"
...
static int __init char_driver_init(void)
{
	...
	alloc_chrdev_region(&dev_num, 0, 1, "tenDevice_p4.2");
	printk("Insert character driver successfully. major(%d), minor(%d)\n", MAJOR(dev_num), MINOR(dev_num));

	/* tao device file /dev/char_device */
	device_class = class_create(THIS_MODULE, "tenClassFileDevice_4.2");
	device_create(device_class, NULL, dev_num, NULL,"tenFileDevice_4.2");
